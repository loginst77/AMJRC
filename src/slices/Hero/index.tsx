...
import { asLink, type Content, isFilled, type RichTextField, type EmbedField, type TitleField } from "@prismicio/client";
...
const Hero: FC<HeroProps> = ({ slice }) => {
  ...
  const hasPrimaryButton = isFilled.link(primary.buttonLink) && !!primaryButtonHref && isFilled.keyText(primary.buttonText);
  const hasSecondaryButton =
    isFilled.link(primary.secondaryButtonLink) && !!secondaryButtonHref && isFilled.keyText(primary.secondaryButtonText);
      ...
      <Container className="relative flex h-full flex-col justify-between pb-12 pt-32 sm:pb-16">
        ...
        <div className="space-y-5">
          <PrismicRichText
            field={primary.text}
            components={{
              paragraph: ({ children }) => <p className="max-w-xl !text-base font-semibold uppercase tracking-[0.2em]">{children}</p>,
            }}
          />
