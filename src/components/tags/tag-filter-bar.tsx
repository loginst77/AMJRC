...
export function TagFilterBar({ allCount, tags, anchorId = "podcast-list", allHref = "/media/podcasts" }: TagFilterBarProps) {
  ...
  const latestDesktopTags = tags.slice(-3);
  const desktopTags = (() => {
    ...
    // Keep 3 visible desktop tags: inject selected one right after "All".
    return [activeTag, ...latestDesktopTags.slice(0, 2)];
  })();
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:gap-3">
            ...
            <div className="inline-flex w-full items-center gap-1 rounded-full bg-zinc-100 p-1 text-zinc-500 lg:hidden">
              ...
              <button
                ...
                className="ml-auto inline-flex items-center gap-2 rounded-full bg-zinc-200 px-4 py-2 text-base text-blue-600 transition-colors hover:bg-zinc-200/70 hover:text-blue-700 font-semibold"
              >
                <FilterIcon className="h-6 w-6" strokeWidth={1.8} />
                <span className="hidden sm:inline">Фильтры</span>
              </button>
            </div>

            {/* Desktop: show "All", up to 3 tags and filters button */}
            <div className="hidden w-full max-w-5xl items-center gap-1 rounded-full bg-zinc-100 p-1 text-zinc-500 lg:inline-flex">
              ...
              <button
                type="button"
                onClick={() => setOpen(true)}
                className="ml-auto inline-flex items-center gap-2 rounded-full bg-zinc-200 px-4 py-2 text-base text-blue-600 transition-colors hover:bg-zinc-200/70 hover:text-blue-700 cursor-pointer font-semibold"
              >
