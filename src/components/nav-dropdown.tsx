        ...
        <div className="grid h-28 grid-flow-col auto-cols-fr items-stretch overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-950">
          {items.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="flex h-full w-full flex-col items-center justify-center gap-4 whitespace-nowrap px-8 text-sm font-medium text-zinc-700 transition-colors hover:bg-blue-100 hover:text-zinc-950 dark:text-zinc-300 dark:hover:bg-zinc-900 dark:hover:text-white"
            >
              {item.icon ? (
